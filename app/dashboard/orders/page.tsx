        ...
        <div className="receipt-print" style={{ display: 'none' }}>
          <div style={{ fontFamily: '"Courier New", Courier, monospace', fontSize: '12px', color: '#000', width: '72mm', padding: '0' }}>
            ...
            <div style={{ textAlign: 'center', paddingBottom: '10px' }}>
              <div style={{ fontWeight: 900, fontSize: '22px', letterSpacing: '3px', marginBottom: '2px' }}>BANANA SUSHI</div>
              <div style={{ fontSize: '10px', letterSpacing: '1px' }}>· · · · · · · · · · · · · · · · · · · ·</div>
              <div style={{ fontSize: '10px', marginTop: '4px' }}>Sushi-Allee 42, 10115 Berlin</div>
              <div style={{ fontSize: '10px' }}>Tel: +49 (0) 30 123 456 78</div>
              <div style={{ fontSize: '10px' }}>www.bananasushi.de</div>
            </div>

            <div style={{ borderTop: '1px dashed #000', margin: '0 0 8px 0' }} />

            {/* Order info */}
            <div style={{ marginBottom: '8px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span style={{ fontWeight: 700 }}>ORDER #</span>
                <span style={{ fontWeight: 700 }}>{selectedOrder.orderNumber}</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '10px', marginTop: '2px' }}>
                <span>{new Date(selectedOrder.createdAt).toLocaleDateString('de-DE')}</span>
                <span>{new Date(selectedOrder.createdAt).toLocaleTimeString('de-DE', { hour: '2-digit', minute: '2-digit' })}</span>
              </div>
            </div>
            ...
            <div style={{ marginBottom: '8px', fontSize: '11px' }}>
              <div style={{ fontWeight: 700, fontSize: '10px', letterSpacing: '1px', marginBottom: '3px' }}>DELIVER TO:</div>
              <div style={{ fontWeight: 700 }}>{selectedOrder.customerName}</div>
              <div>{selectedOrder.phone}</div>
              <div>{selectedOrder.address}</div>
              <div>{selectedOrder.zipCode} {selectedOrder.city}</div>
              {selectedOrder.deliveryNote && (
                <div style={{ marginTop: '4px', fontSize: '10px', fontStyle: 'italic' }}>Note: {selectedOrder.deliveryNote}</div>
              )}
            </div>
            ...
            {/* Items header */}
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '10px', fontWeight: 700, letterSpacing: '1px', marginBottom: '4px' }}>
              <span>ITEM</span>
              <span>TOTAL</span>
            </div>
              ...
              {selectedOrder.items.map((item: any, i: number) => (
                <div key={i} style={{ marginBottom: '3px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12px' }}>
                    ...
                    <span style={{ whiteSpace: 'nowrap' }}>{(item.price * item.quantity).toFixed(2)} EUR</span>
                  </div>
            ...
            <div style={{ marginBottom: '8px' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px', marginBottom: '4px' }}>
                ...
                <span>{selectedOrder.deliveryFee.toFixed(2)} EUR</span>
              </div>
            ...
            <div style={{ marginBottom: '10px', fontSize: '11px' }}>
              ...
              {selectedOrder.paymentMethod === 'online' && (
                <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '2px', fontSize: '10px' }}>
                  <span>Status</span>
                  <span style={{ fontWeight: 700 }}>PAID ✓</span>
                </div>
              )}
            </div>

            <div style={{ borderTop: '1px dashed #000', margin: '0 0 10px 0' }} />

            {/* Footer */}
            <div style={{ textAlign: 'center', fontSize: '10px', lineHeight: '1.6' }}>
              <div>Vielen Dank für Ihre Bestellung!</div>
              <div>Thank you for your order!</div>
              <div style={{ marginTop: '6px', letterSpacing: '1px' }}>* * * * * * * * * * * * * * * *</div>
              <div style={{ marginTop: '4px', fontSize: '9px', color: '#555' }}>
                {selectedOrder.orderNumber} — {new Date(selectedOrder.createdAt).toLocaleDateString('de-DE')}
              </div>
            </div>
